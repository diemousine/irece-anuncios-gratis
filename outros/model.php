...
class _model extends MODEL {
	...
	public static function registrar($dados, $img) {
		...
		// TIPO DE PRODUTO
		if(isset($dados['produto']) && ($outros = mysqli_real_escape_string($con, trim(filter_var($dados['produto'], 513)))) == '') {
			mysqli_close($con);
		...
		// KEYWORDS
		$keywords = $transacao.' '.$outros.' '.$uf.' '.$cidade.' '.$valor.' '.$obs.' ';

		$query = "INSERT INTO outros(codusuario, transacao, tipo, uf, cidade, valor, contato, telefone, observacao, keywords, dtcadastro) VALUES (".$_SESSION['codusuario'].", '$transacao', '$outros', '$uf', '$cidade', $valor, '".$_SESSION['nome']."', '".$_SESSION['telefone']."', '$obs', '$keywords', '".date('Y-m-d', time())."')";

		mysqli_query($con, $query);

		if(mysqli_affected_rows($con) > 0) {
			$codoutros = mysqli_insert_id($con);
			$idoutros = $codoutros.time();

			// IMAGEM
			if(isset($img['imagem']) && !empty($img['imagem']['name'])) {
				include_once '../classes/com.imagem.php';
				$consul = IMAGEM::salvar('../img/outros/', $img);
				if($consul[0] === true) {
					$imagem = $consul[1];
					$destino = $consul[2];
				} else {
					mysqli_query($con, "DELETE FROM outros WHERE codoutros = $codoutros");
					mysqli_close($con);
					return $consul[1];
				}
			} else {
			...
			mysqli_query($con, "UPDATE outros SET idoutros = $idoutros, imagem = '$imagem' WHERE codoutros = $codoutros");
			if(mysqli_affected_rows($con) <= 0) {
	...
	public static function publicacoes($dados, $pagina) {
		$con = bdcon();
		$resul = "";
		if(($outros = self::listar('outros', $dados, $pagina)) !== false) {
			$count = 0;
			while($row = mysqli_fetch_assoc($outros)) {
				$imgsize = @getimagesize("../img/outros/".$row['imagem']);
				$tamanho = ($imgsize[0] >= $imgsize[1]) ? 'width' : 'height';
				$resul .= (($count % 2) == 0) ? "<div class='row'>" : ""; // CRIA UMA NOVA LINHA PARA CADA 2 PUBLICACÕES
				$resul .= ("
					<div class='col-md-6'>
						<div class='panel panel-primary'>
							<div class='panel-heading'><strong>".mb_strtoupper($row['tipo'], 'UTF-8')." PARA ".$row['transacao']." #".$row['idoutros']."</strong></div>
							<div class='panel-body' style='overflow: hidden'>
								<center><img src='../img/outros/".$row['imagem']."' ".$tamanho."='100%' alt='miniatura'></center>
				...
				$query = "UPDATE outros SET outros.visitas = (outros.visitas+1) WHERE codoutros = ".$row['codoutros'];
				mysqli_query($con, $query);
